...
describe('POST /api/settings/review-mode', () => {
  ...
  it('schedules task creation for pending emails when disabling manual review', async () => {
    ...
    expect(mockCreateTask).toHaveBeenNthCalledWith(1, 'user-1', 'email-1', 'ai_suggestion')
    expect(mockCreateTask).toHaveBeenNthCalledWith(2, 'user-1', 'email-2', 'ai_suggestion')
  })
})

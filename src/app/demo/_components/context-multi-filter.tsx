...
// Demo-local ContextMultiFilter — mirrors the real product's filter at
// src/app/dashboard/page.tsx (L1529-1606). Same Popover trigger UX, same
// "All / 1 selected / N selected" trigger text, same Check-iconned checkbox
// rows. Kept inside the demo folder because the real one isn't exported.

import type { ReactNode } from 'react'
import { Check } from 'lucide-react'
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover'

export interface ContextMultiOption {
  id: string
  name: string
}
      ...
      >
        <span className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-slate-900">
            {disabled ? 'Select identity first' : triggerText}
          </span>
        </span>
      </PopoverTrigger>
      <PopoverContent
        ...
        <div className="max-h-64 overflow-y-auto">
          {options.length === 0 ? (
            <p className="px-2.5 py-3 text-sm text-slate-400">
              No related {label.toLowerCase()}s yet.
            </p>
          ) : (
            options.map((option) => (
              <button
